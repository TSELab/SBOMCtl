...
def process_policy(policy_name: str):
    # build input path from policy name
    read_from = os.path.join(results_dir, f"performance_{policy_name}.json")
    performance_data = []  
    ...
    output_dir = os.path.join(results_dir, policy_name)
    os.makedirs(output_dir, exist_ok=True)

    def save_violin_plot(data, y, x, ylabel, xlabel, palette, ylim=None, xtick_interval=None):
        plt.figure(figsize=(8, 6))
        sns.violinplot(data=data, y=y, x=x, cut=0, palette=palette, hue=x , legend=False)
